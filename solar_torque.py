...
if any(~npm[:6]) | any(~npm[-6:]):
    raise StandardError('Timeframe must start and end in Normal Point mode + 25 minute pad.')
i1_npm = ~npm[:-5] & npm[1:-4] & npm[2:-3] & npm[3:-2] & npm[4:-1] & npm[5:] # transition must be followed by 20 min NPM
i2_npm = npm[:-5] & npm[1:-4] & npm[2:-3] & npm[3:-2] & npm[4:-1] & ~npm[5:]  # transition must be preceded by 20 min NPM
i1_npm = append(append(zeros(3, dtype='bool'), i1_npm), zeros(2, dtype='bool')) # delay start time by 10 min
i2_npm = append(append(zeros(3, dtype='bool'), i2_npm), zeros(2, dtype='bool')) 
